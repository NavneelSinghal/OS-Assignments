...
main (void)
{
  ...
  TITLE ("TEST 7: Seeking one character ahead\n");
  fd = open ("test0");
  seek (fd, 1);
  bytes_read = read (fd, sbuf, READ_SIZE);
  if (bytes_read < 0 || (size_t)bytes_read != strlen (testdata) - 1)
    {
      ERROR ("Failed to read %d bytes from file, read %d.\n",
             strlen (testdata), bytes_read);
    }
  ...
  TITLE ("TEST 8: Filesize\n");
  fd = open ("test0");
  int fs = filesize (fd);
  if (fs != strlen (testdata))
    {
      ERROR ("Wrong filesize: %d, expected: %d\n", fs, strlen (testdata));
    }
  SUCCESS ("TEST 8: Passed\n");

  TITLE ("TEST 9: Removing file\n");
  bool r = remove ("test0");
  if (r)
    {
      fd = open ("test0");
      if (fd == -1)
        ...
        {
          ERROR ("Opened a removed file.\n");
        }
    }

  TITLE ("If there is no text after this, the test for halt is successful.\n");

  halt();

  ERROR ("Halt unsuccessful.\n");

  TITLE ("The test suite should now exit. Since SYS_WAIT is not implemented "
         "yet, the program should hang. ");
